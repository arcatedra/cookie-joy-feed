...
BEGIN
  ...
  IF is_admin THEN
    ...
    RETURN NEW;
  END IF;
  IF NEW.status IS DISTINCT FROM OLD.status THEN RAISE EXCEPTION 'businesses.status is admin-only'; END IF;
  IF NEW.rejection_reason IS DISTINCT FROM OLD.rejection_reason THEN RAISE EXCEPTION 'businesses.rejection_reason is admin-only'; END IF;
  IF NEW.approved_at IS DISTINCT FROM OLD.approved_at OR NEW.approved_by IS DISTINCT FROM OLD.approved_by THEN
    RAISE EXCEPTION 'businesses approval bookkeeping is admin-only'; END IF;
  IF NEW.owner_user_id IS DISTINCT FROM OLD.owner_user_id THEN RAISE EXCEPTION 'businesses.owner_user_id is read-only'; END IF;
  RETURN NEW;
END; $$;
...
SELECT id, business_name, business_type, city, logo_url, status, created_at
FROM public.businesses WHERE status = 'aprobado';

CREATE POLICY "Public views approved businesses" ON public.businesses FOR SELECT TO anon
...
CREATE POLICY "Public views products of approved businesses" ON public.business_products FOR SELECT TO anon, authenticated
  USING (is_active = TRUE AND business_id IN (SELECT id FROM public.businesses WHERE status = 'aprobado'));
CREATE POLICY "Owners view own products" ON public.business_products FOR SELECT TO authenticated
  USING (business_id IN (SELECT id FROM public.businesses WHERE owner_user_id = auth.uid()));
CREATE POLICY "Admins view all products" ON public.business_products FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Approved owners manage their products" ON public.business_products FOR ALL TO authenticated
  USING (business_id IN (SELECT id FROM public.businesses WHERE owner_user_id = auth.uid() AND status = 'aprobado'))
  WITH CHECK (business_id IN (SELECT id FROM public.businesses WHERE owner_user_id = auth.uid() AND status = 'aprobado'));
CREATE POLICY "Admins manage any products" ON public.business_products FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::app_role))
  WITH CHECK (public.has_role(auth.uid(), 'admin'::app_role));
...
CREATE POLICY "Public views active offers of approved businesses" ON public.business_offers FOR SELECT TO anon, authenticated
  USING (is_active = TRUE AND business_id IN (SELECT id FROM public.businesses WHERE status = 'aprobado'));
CREATE POLICY "Owners view own offers" ON public.business_offers FOR SELECT TO authenticated
  USING (business_id IN (SELECT id FROM public.businesses WHERE owner_user_id = auth.uid()));
CREATE POLICY "Admins view all offers" ON public.business_offers FOR SELECT TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Approved owners manage their offers" ON public.business_offers FOR ALL TO authenticated
  USING (business_id IN (SELECT id FROM public.businesses WHERE owner_user_id = auth.uid() AND status = 'aprobado'))
  WITH CHECK (business_id IN (SELECT id FROM public.businesses WHERE owner_user_id = auth.uid() AND status = 'aprobado'));
CREATE POLICY "Admins manage any offers" ON public.business_offers FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'::app_role))
  WITH CHECK (public.has_role(auth.uid(), 'admin'::app_role));
